...
exports.mergeSearchBody = function (searchOpts) {
    ...
    if (searchOpts.filter && Object.keys(searchOpts.filter).length) {
      body.filter = searchOpts.filter;  
    } else if (searchOpts.where && Object.keys(searchOpts.where).length) {
        var mustFilters = []
        var shouldFilters = []
...
exports.elasticsearchBodyOk = function (elasticsearchBody) {
    ...
    return elasticsearchBody && (elasticsearchBody.ok || elasticsearchBody.acknowledged || (elasticsearchBody._shards && elasticsearchBody._shards.failed === 0))
}
